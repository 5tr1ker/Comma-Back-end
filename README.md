...
- 사용자 : 회원가입 및 로그인 , 소셜 계정 로그인 (구글, 네이버 , 카카오), 회원정보 수정 , 친구 추가
- 음악 : 음악 재생
- 탐색 : 사용자 , 가수 , 음악 검색 기능
- 아카이브 : 현재 재생되는 음악에 대한 감정 기록
- 플레이리스트 : 음악 추가 및 편집 , 알림 시간 설정 , 익명 또는 친구에게 플레이리스트 추천
- 알림 : 알림 시간에 맞춰 실시간 푸시 알림 기능 구현
- 마이페이지 : 내 플레이리스트 , 최애 가수 및 카테고리 , 아카이브 작성 기록 , 친구 목록
